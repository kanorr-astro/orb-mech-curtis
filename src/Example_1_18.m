...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%{
This function uses the RK1 through RK4 methods with two different time
steps each to solve for and plot the response of a damped single degree of
freedom spring-mass system to a sinusoidal forcing function, represented
by:

x'' + 2*z*wn*x' + wn^2*x = (Fo/m)*sin(w*t)

The numerical integration is done by the external function 'rk1_4', which
uses the subjunction 'rates' herein to compute the derivatives.
...
m               -mass (kg)
z               -dimensionless damping factor
w               -forcing frequency
t0              -initial time (s)
tf              -final time (s)
...
t1,....,t4      -solution times for RK1,....,RK4 for smaller h
...
%...Solve using RK1 through RK4, using the same and a larger time step for
%each method:
rk = 1;
...
%------------------------
%{
This function calculates first and second time derivates of x as governed
by the equation:
x'' + 2*z*wn*x' + wn^2*x = (Fo/m)*sin(w*t)
