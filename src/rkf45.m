...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%{
This function uses the Runge-Kutta-Fehlberg 4(5) algorithm to integrate a
system of first-order differential equations dy/dt = f(t,y).

y               -column vector of solutions
f               -column vector of the derivatives dy/dt
t               -time (s)
a               -Fehlberg coefficients for locating the six solution points
                (nodes) within each time interval
b               -Fehlberg coupling coefficients for computing the derivatives
...
ode_function    -handle for user M-function in which the derivatives f are
                computed
...
yout            -a matrix, each row of which contains the componenets of y
                evaluated at the corresponding time in tout
h               -time step (s)
hmin            -minimum allowable time step (s)
ti              -time at the beginning of a time step
yi              -values of y at the beginning of a time step
t_innner        -time within a given time step
...
tol             -relative tolerance (used above?)
delta           -fractional change in step size
eps             -unit roundoff error (smallest number for which 1+eps > 1)
...
while t < tf
    ...
    te      = h*f*(c4'- c5');   %Difference betweeen 4th/5th order solutions
