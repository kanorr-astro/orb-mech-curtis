...
%~~~~~~~~~~~~~~~~~~~
%{
This function uses the predictor-corrector method to integrate a system of
first-order differential equations dy/dt = f(t,y)

y               -column vector of solutions
f               -column vector of the derivatives dy/dt
ode_function    -handle for the user M-fuction in which the derivatives of
                f are computed
t               -time
t0              -initial time
tf              -final time
...
y2p             -predicted value of y at the beginning of a time step
y2              -corrected value of y at the end of a time step
err             -maximum relative error (for all componenets) between y2p
                and y2 for given iteration
eps             -unit roundoff error (the smallest number for which 1+eps >
                1). Used to avoid a zero denominator
