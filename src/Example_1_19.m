...
%~~~~~~~~~~~~~~~~~~~~~~~~~~
%{
This program uses Heun's method with two different time steps to solve for
and plot the response of a damped single degree of freedom spring-mass
system to a sinusoidal forcing function, represented by:

x'' + 2*z*wn*x' + wn^2*x = (Fo/m)*sin(w*t)

The numerical integration is done in the external function 'heun', which
uses the subfunction 'rates' herein to compute the derivatives.

x           -displacement (m)
'           -shorthand for d/dt
t           -time (s)
wn          -natural circular frequency (rad/s)
z           -damping factor
Fo          -amplitude of the sinusoidal forcing function (N)
m           -mass (kg)
w           -forcing frequency (rad/s)
t0          -initial time (s)
tf          -final time (s)
...
%...Calculate and plot the RK3 solution for h = 0.1;
h       = 0.1;
rk      = 3;
...
%~~~~~~~~~~~~~~~~~~~~~
%{
This function calculates the first and second time derivatives of x for the
forced vibration of a damped single degree of freedom system represented by
the 2nd-order differential equation:

x'' + 2*z*wn*x' + wn^2*x = (Fo/m)*sin(w*t)

Dx          -velocity (m/s)
D2x         -acceleration (m/s^2)
...
hold on
plot(t3, f3(:,1), '-b', 'LineWidth', 0.75)
legend('h = 1.0', 'h = 0.1', 'rk3/h = 0.1')
end %output
